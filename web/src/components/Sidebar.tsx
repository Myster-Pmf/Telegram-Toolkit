...
export default function Sidebar() {
    return (
        <aside className="w-16 flex flex-col bg-[var(--color-bg-panel)] border-r border-[var(--color-border)]">
            {/* Logo */}
            ...
            <nav className="flex-1 py-3 flex flex-col gap-1">
                {navItems.map((item) => (
                    <NavLink
                        ...
                        to={item.path}
                        className={({ isActive }) =>
                            `mx-2 h-10 flex items-center justify-center rounded-md transition-colors ${isActive
